...
w = 2*pi;
A = 2;
generate_plot_for_delta_variation = false;
generate_plot_for_link_angle_variation = false;
generate_plot_for_link_angle_der_variation = true;
generate_plot_for_delta_der_variation = false;
...
elseif generate_plot_for_link_angle_der_variation == true
    delta = deg2rad(3);
    Z = zeros([9 length(t)]);
    for i = 1:1:9
        Z(i,:) = derivative_novel(t,A,w,i,delta);
    end
    for i = 1:1:9
        plot(t,Z(i,:));
        grid on;
    ...
    title("Variation of Link Angles")
    hold on;
elseif generate_plot_for_delta_der_variation == true
   delta = [2,8,16,32];
   delta = deg2rad(delta) ;
   Z = zeros([length(delta) length(t)]);
   for i = 1:1:length(delta)
         Z(i,:) = derivative_novel(t,A,w,2,delta(i));
         plot(t,Z(i,:));
         grid on;
         hold on;
   end
